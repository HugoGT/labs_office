...
HTTP routes (all in `server/src/createOfficeServer.ts`): `GET /health`, `POST /livekit/token`, `GET /spaces`, `GET /desks`, `POST /desks/:id/claim`, `GET|POST /me/desk`, `POST /me/desk/release`, `GET /assets`, `POST /recordings/{start,stop,url}`, and `/admin/*` (including `GET /admin/users` and `POST /admin/users/:id/revoke`).
